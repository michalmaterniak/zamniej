<template>
  <div>
    <div v-if="token && enable" v-html="toolbar"></div>
  </div>
</template>
<script>
export default {
  ...
  computed: {
    historyAjax()
    {
      return this.$store.getters.debug.ajax;
    },
    token() {
      return this.$store.getters.debug.token;
    }
  },
  methods:{
    init() {
      if(process.env.NODE_ENV === 'development') {
        ...
        let Sfjs = (function () {
          ...
          var requestStack = [];
          var extractHeaders = function (xhr, stackElement) {            /* Here we avoid to call xhr.getResponseHeader in order to */            /* prevent polluting the console with CORS security errors */
          ...
          var renderAjaxRequests = function () {
            ...
            requestCounter.textContent = requestStack.length-1;
            var infoSpan = document.querySelector(".sf-toolbar-ajax-info");
            if (infoSpan) {
              infoSpan.textContent = requestStack.length-1 + ' AJAX request' + (requestStack.length !== 1 ? 's' : '');
            }
          ...
          var startAjaxRequest = function (index) {
            ...
            requestNumberCell.textContent = index;
          ...
          if (window.XMLHttpRequest && XMLHttpRequest.prototype.addEventListener) {
            ...
            XMLHttpRequest.prototype.open = function (method, url, async, user, pass) {
              ...
              } else if (0 === url.indexOf(process.env.DEV_HOST)) {
                path = url.substr(18);
              }
          ...
          return {
            ...
            loadToolbar: function (token, newToken) {
              ...
              this.load('sfwdt' + token, 'http\u003A\/\/zamniej.loc\/_wdt\/xxxxxx'.replace(/xxxxxx/, newToken), function (xhr, el) {                        /* Evaluate in global scope scripts embedded inside the toolbar */
            ...
            createFilters: function () {
              document.querySelectorAll('[data-filters] [data-filter]').forEach(function (filter) {
                var filters = filter.closest('[data-filters]'), type = 'choice', name = filter.dataset.filter,
                  ucName = name.charAt(0).toUpperCase() + name.slice(1), list = document.createElement('ul'),
                  values = filters.dataset['filter' + ucName] || filters.querySelectorAll('[data-filter-' + name + ']'),
                  labels = {}, defaults = null, indexed = {}, processed = {};
  ...
  mounted() {
    ...
    this.axios({
      method: "GET",
      url: this.baseUrl + '/_wdt/' + this.token,
      responseType: 'text',
    }).then(res => {
