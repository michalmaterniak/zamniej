  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="row mt-5">
        ...
        <div class="col-md-11">
          <input type="text" class="form-control border-black" placeholder="Url" v-model="url" @input="convertUrlTradetracker">
...
export default {
  ...
  props: ['title', 'data'],
  name: "PopupOffersNewTradetracker",
  data() {
    return {
      forceUrl: false,
      url: '',
  ...
  methods: {
    convertUrlTradetracker() {
      this.ajax({
        url: '/admin/api/affiliations/tradetracker/programs-convertUrl/' + this.shopAffiliationOffer,
        data: {
          url: this.url
        },
        responseCallbackSuccess: res => {
          if(res.data.success === true) {
            this.trackingUrlOffer = res.data['tracking-url-tradetracker'];
          }
        }
      })
    },
    addOffer() {
      if(this.isValidate) {
        this.ajax({
          url: '/admin/api/affiliations/tradetracker/vouchers/createCustomOffer',
          data: {
            offer: {
