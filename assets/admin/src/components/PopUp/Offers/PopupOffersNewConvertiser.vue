  ...
  <div class="modal-content">
    ...
    <div class="modal-body">
      ...
      <div class="row mt-5">
        ...
        <div class="col-md-11">
          <input type="text" class="form-control border-black" placeholder="Url" v-model="url" @input="convertUrlConvertiser">
...
export default {
  ...
  props: ['title', 'data'],
  name: "PopupOffersNewConvertiser",
  data() {
    return {
      forceUrl: false,
      url: '',
  ...
  methods: {
    convertUrlConvertiser() {
      this.ajax({
        url: '/admin/api/affiliations/convertiser/programs-convertUrl',
        data: {
          url: this.url
        },
        responseCallbackSuccess: res => {
          if(res.data.success === true) {
            this.trackingUrlOffer = res.data['tracking-url-convertiser'];
          }
        }
      })
    },
    addOffer() {
      if(this.isValidate) {
        this.ajax({
          url: '/admin/api/affiliations/convertiser/vouchers/createCustomOffer',
          data: {
            offer: {
