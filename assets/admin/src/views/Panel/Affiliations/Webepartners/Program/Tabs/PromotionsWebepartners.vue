...
export default {
  ...
  methods:{
    createOffer(offer)
    {
      ...
      {
        this.ajax({
          url: '/admin/api/affiliations/webepartners/promotions/createOffer',
          data:{
            promotions: [offer.idOffer]
          },
    ...
    {
      this.ajax({
        url: '/admin/api/affiliations/webepartners/promotions/' + this.program.idShop + this.fragmentUrl,
        responseCallbackSuccess: res => {
          this.countAll = res.data.count;
          this.offers = res.data.promotions;
        }
      })
    }
  },
