      ...
      <tbody>
        <tr v-for="(offer, index) in offers" :key="index">
          ...
          <td>
            ...
            <button v-if="offer.offer" class="btn btn-info" @click="addToSlider(offer.offer)">
...
export default {
  ...
  methods:{
    createOffer(offer)
    {
      ...
      {
        this.ajax({
          url: '/admin/api/affiliations/webepartners/banners/createOffer',
          data:{
            banners: [offer.idOffer]
          },
          responseCallbackSuccess: res => {
            this.showAlert(res.data.message, 'success');
            offer.offer = res.data.banners[offer.idOffer];
          }
        })
      }
    },
    ...
    {
      this.ajax({
        url: '/admin/api/affiliations/webepartners/banners/' + this.program.idShop + this.fragmentUrl,
        responseCallbackSuccess: res => {
          this.countAll = res.data.count;
          this.offers = res.data.banners;
        }
      })
    }
  },
