<template>
  <div v-if="program">
    ...
    <details-tradetracker-program :program="program" />
  </div>
</template>

<script>
import affiliations from "../../../../mixins/Affiliations/affiliations";
import DetailsTradetrackerProgram from "./Program/DetailsTradetrackerProgram";
import BasicTradetracker from "./Program/Tabs/BasicTradetracker";

export default {
  components: {DetailsTradetrackerProgram},
  props:['id'],
  name: "ProgramTradetracker",
  mixins:[affiliations],
  data(){
    return {
      program: null
  ...
  methods:{
    update() {
      // this.ajax({
      //   url: '/admin/api/affiliations/tradetracker/programs-update/' + this.program.idShop,
    ...
    getProgram(){
      this.ajax({
        url: '/admin/api/affiliations/tradetracker/programs-program/' + this.id,
        responseCallbackSuccess: res => {
          this.program = res.data.program;
        }
      })
